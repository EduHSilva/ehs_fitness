...
class WorkoutService {
  String get _workoutsCacheKey =>
      'cached_workouts_${AppConfig.user?.id ?? 'guest'}';

  Future<List<Workout>> _cachedWorkouts() async {
    final saved =
        (await SharedPreferences.getInstance()).getString(_workoutsCacheKey);
  ...
  Future<List<Exercise>> fetchExercises() async {
    ...
    final response = await client
        .get(Uri.parse('${AppConfig.apiUrl}/fitness/workout/exercises'));
    ...
    if (response.statusCode == 200) {
      ...
      if (data['data'] != null) {
        exercises = List<Exercise>.from(
            data['data'].map((exe) => Exercise.fromJson(exe)));
      }
  ...
  Future<List<Workout>> fetchWorkouts() async {
    try {
      ...
      await (await SharedPreferences.getInstance())
          .setString(_workoutsCacheKey, response.body);
  ...
  Future<WorkoutResponse> getWorkout(int id) async {
    http.Client client = await AppConfig.getHttpClient();
    final response = await client
        .get(Uri.parse('${AppConfig.apiUrl}/fitness/workout?id=$id'));

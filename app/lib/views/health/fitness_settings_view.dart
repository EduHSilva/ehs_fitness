...
class _FitnessSettingsViewState extends State<FitnessSettingsView> {
  ...
  Future<void> _save() async {
    ...
    final settings = HomeSettings(
        dailyCardioMinutes: int.tryParse(_daily.text) ?? 0,
        weeklyCardioMinutes: int.tryParse(_weekly.text) ?? 0,
        creatineEnabled: _creatine);
    try {
            ...
            : ListView(padding: const EdgeInsets.all(16), children: [
                ...
                TextField(
                    ...
                    decoration: const InputDecoration(
                        ...
                        helperText: 'Use 0 para ocultar a seção de cardio.')),
                ...
                SwitchListTile(
                    ...
                    title: const Text('Exibir creatina na home'),
                    value: _creatine,
                    onChanged: (value) => setState(() => _creatine = value)),
                const SizedBox(height: 24),
                FilledButton.icon(
                    onPressed: _saving ? null : _save,
                    icon: const Icon(Icons.save_outlined),
                    label:
                        Text(_saving ? 'Salvando...' : 'Salvar preferências')),
              ]),
      );
}

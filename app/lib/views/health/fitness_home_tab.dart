...
class _FitnessHomeTabState extends State<FitnessHomeTab> {
  ...
  String get _storageKey =>
      '${_storagePrefix}_${AppConfig.user?.id ?? 'guest'}';
  ...
  Future<void> _load() async {
    ...
    if (saved != null) {
      ...
      _records.addAll(decoded.map((key, value) =>
          MapEntry(key, _DailyRecord.fromJson(value as Map<String, dynamic>))));
    }
    final homeSettingsService = HomeSettingsService();
    try {
      _settings = await homeSettingsService.fetch();
    } catch (_) {}
    try {
      final history = await homeSettingsService.fetchTodayHistory();
      if (history != null) {
        final today = DateUtils.dateOnly(DateTime.now());
        final current = _recordFor(today);
        _records[_dateKey(today)] = current.copyWith(
          cardioMinutes: history.cardio,
          creatine: history.creatine,
        );
        await _save();
      }
    } catch (_) {
      // The locally cached record remains available when the API is offline.
    }
  ...
  Future<void> _save() async {
    ...
    await preferences.setString(
        _storageKey,
        jsonEncode(
            _records.map((key, value) => MapEntry(key, value.toJson()))));
  }
  ...
  int get _weekCardio => _weekDates.fold(
      0, (total, date) => total + _recordFor(date).cardioMinutes);
  ...
  Future<void> _editSettings() async {
    final value = await Navigator.push<HomeSettings>(context,
        MaterialPageRoute(builder: (_) => const FitnessSettingsView()));
            ...
            ListTile(
                ...
                onTap: () {
                  ...
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const WorkoutPlanView()));
                }),
            ListTile(
                ...
                onTap: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text('A geração por IA será conectada à API.')));
                }),
            ListTile(
                leading: const Icon(Icons.picture_as_pdf_outlined),
                title: const Text('Importar plano por PDF'),
                onTap: () {
                  Navigator.pop(sheetContext);
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text(
                          'A importação de PDF será processada pela API.')));
                }),
          ])));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          actions: [
            ...
            IconButton(
                ...
                onPressed: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            UserProfileView(id: AppConfig.user!.id))),
      ...
      body: Column(children: [
        ...
        if (_settings.showCardio)
          Padding(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 0),
              child: _CardioProgress(
                  daily: _recordFor(_selectedDate).cardioMinutes,
                  dailyGoal: _settings.dailyCardioMinutes,
                  weekly: _weekCardio,
                  weeklyGoal: _settings.weeklyCardioMinutes)),
        Expanded(
            child: _weeklyView
                ? ListView.builder(
                    padding: const EdgeInsets.all(16),
                    itemCount: _weekDates.length,
                    itemBuilder: (_, i) => _DailyRecordCard(
                        date: _weekDates[i],
                        record: _recordFor(_weekDates[i]),
                        showCardio: _settings.showCardio,
                        showCreatine: _settings.creatineEnabled,
                        onChanged: (record) => _update(_weekDates[i], record)))
                : Padding(
                    padding: const EdgeInsets.all(16),
                    child: _DailyRecordCard(
                        ...
                        showCreatine: _settings.creatineEnabled,
                        onChanged: (record) =>
                            _update(_selectedDate, record)))),
      ]),
      floatingActionButton: FloatingActionButton(
          onPressed: _showActions, child: const Icon(Icons.add)),
    );
  }
}

class _CardioProgress extends StatelessWidget {
  const _CardioProgress(
      {required this.daily,
  ...
  Widget build(BuildContext context) => Card(
      child: Padding(
          ...
          child:
            ...
            const Text('Progresso de cardio',
                style: TextStyle(fontWeight: FontWeight.bold)),
            if (dailyGoal > 0)
              _Progress(
                  label: 'Hoje: $daily / $dailyGoal min',
                  value: daily / dailyGoal),
            if (weeklyGoal > 0)
              _Progress(
                  label: 'Semana: $weekly / $weeklyGoal min',
                  value: weekly / weeklyGoal),
          ])));
}
...
class _DailyRecordCard extends StatelessWidget {
  const _DailyRecordCard(
      ...
      required this.showCardio,
      required this.showCreatine,
      required this.onChanged});
  final DateTime date;
  final _DailyRecord record;
  final bool showCardio;
  final bool showCreatine;
  final ValueChanged<_DailyRecord> onChanged;
  Future<void> _recordCardio(BuildContext context) async {
    final controller = TextEditingController(
        text: record.cardioMinutes > 0 ? record.cardioMinutes.toString() : '');
  ...
  Widget build(BuildContext context) => Card(
      ...
      child: Column(children: [
        Container(
            ...
            padding: const EdgeInsets.all(12),
            child: Text(DateFormat.EEEE('pt_BR').format(date),
                style: const TextStyle(fontWeight: FontWeight.bold))),
        ListTile(
            leading: const Icon(Icons.fitness_center, color: AppColors.primary),
            title: const Text('Treino do dia'),
            subtitle: Text(record.workoutDone
                ? 'Concluído'
                : 'Adicione o treino do seu plano'),
            trailing: Checkbox(
                value: record.workoutDone,
                onChanged: (v) =>
                    onChanged(record.copyWith(workoutDone: v ?? false))),
            onTap: () =>
                onChanged(record.copyWith(workoutDone: !record.workoutDone))),
        if (showCardio) ...[
          const Divider(height: 1),
          SwitchListTile(
              secondary: IconButton(
                  tooltip: 'Registrar com timer',
                  icon: const Icon(Icons.timer_outlined,
                      color: AppColors.primary),
                  onPressed: () => _timer(context)),
              title: const Text('Cardio'),
              subtitle: Text(record.cardioMinutes > 0
                  ? '${record.cardioMinutes} min'
                  : 'Não'),
              value: record.cardioMinutes > 0,
              onChanged: (value) => value
                  ? _recordCardio(context)
                  : onChanged(record.copyWith(cardioMinutes: 0)))
        ],
        if (showCreatine) ...[
          const Divider(height: 1),
          SwitchListTile(
              ...
              onChanged: (v) => onChanged(record.copyWith(creatine: v)))
        ],
      ]));
}
...
class _DailyRecord {
  ...
  factory _DailyRecord.fromJson(Map<String, dynamic> json) => _DailyRecord(
      cardioMinutes: (json['cardioMinutes'] as num?)?.toInt() ??
          (json['cardio'] == true ? 1 : 0),
      creatine: json['creatine'] == true,
      workoutDone: json['workoutDone'] == true);
  _DailyRecord copyWith(
          {int? cardioMinutes, bool? creatine, bool? workoutDone}) =>
      _DailyRecord(
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        OutlinedButton.icon(
            ...
            label: Text(_watch.isRunning ? 'Pausar' : 'Iniciar')),
        FilledButton(
            onPressed: () => Navigator.pop(context, (seconds / 60).ceil()),
            child: const Text('Salvar')),
      ],
    );
  }
